...
with open("examples/voice_01.wav", "rb") as spk, open("examples/emo_sad.wav", "rb") as emo:
    r = requests.post(
        "http://localhost:8000/api/v1/tts",
        files={"spk_audio": spk, "emo_audio": emo},
        data={
...
请求使用 `multipart/form-data` 格式。参数分为两类：

**文件类**——curl 用 `-F "name=@filepath"` 传入，Python 放入 `requests.post(files={...})`：

| 参数 | 类型 | 必填 | 说明 |
|------|------|------|------|
| `spk_audio` | file | 是 | 说话人参考音频（WAV） |
| `emo_audio` | file | 否 | 情感参考音频（WAV） |
